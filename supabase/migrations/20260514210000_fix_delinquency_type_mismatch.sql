-- Normalize legacy integer columns and harden delinquency RPC types.

-- 1) Sanitize invalid values before type conversion.
UPDATE public.payment_plan_concepts
SET due_day = NULL
WHERE due_day IS NOT NULL
  AND (due_day < 1 OR due_day > 31);

UPDATE public.payment_plan_concepts
SET due_month = NULL
WHERE due_month IS NOT NULL
  AND (due_month < 1 OR due_month > 12);

-- 2) Normalize due_day/due_month to smallint for compatibility.
ALTER TABLE public.payment_plan_concepts
  ALTER COLUMN due_day TYPE smallint USING due_day::smallint;

ALTER TABLE public.payment_plan_concepts
  ALTER COLUMN due_month TYPE smallint USING due_month::smallint;

-- 3) Ensure range checks exist (idempotent).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_plan_concepts_due_day_check'
      AND conrelid = 'public.payment_plan_concepts'::regclass
  ) THEN
    ALTER TABLE public.payment_plan_concepts
      ADD CONSTRAINT payment_plan_concepts_due_day_check
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_plan_concepts_due_month_check'
      AND conrelid = 'public.payment_plan_concepts'::regclass
  ) THEN
    ALTER TABLE public.payment_plan_concepts
      ADD CONSTRAINT payment_plan_concepts_due_month_check
      CHECK (due_month IS NULL OR (due_month BETWEEN 1 AND 12));
  END IF;
END $$;

-- 4) Recreate shared delinquency core with explicit casts to avoid
-- "structure of query does not match function result type" in legacy schemas.
...
-- 5) Recompile dependent RPCs against corrected core.
