...
if (typeof addEventListener === "function") {
  addEventListener("unhandledrejection", (e: any) => {
    console.error("[import-bachillerato-grades] unhandledrejection:", e?.reason ?? e);
    e?.preventDefault?.();
  });
  addEventListener("error", (e: any) => {
    console.error("[import-bachillerato-grades] uncaught error:", e?.error ?? e?.message ?? e);
    e?.preventDefault?.();
  });
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: adminRole } = await supabaseAdmin
      ...
      .eq("user_id", requestingUser.id).eq("role", "admin").maybeSingle();
    ...
    const yearMatch = (years ?? []).find(
      (y) => (y.year_range || "").replace(/\s/g, "") === normalizedTarget,
    );
    if (!yearMatch) {
      return json({ error: `No existe el año escolar ${yearRangeInput} para este colegio` }, 400);
    }
    ...
    for (const item of items) {
      ...
      try {
        if (item.grade_value === null || item.grade_value === undefined || item.grade_value === "") {
          summary.omitidos++;
        ...
        // 3) Resolve subject by exact name
        ...
        // 4) Find assignment matching (subject, year, school, section)
        ...
        let assignmentId: string | null =
          (asgs ?? []).find((a) => a.section_id === sectionId)?.id ?? null;

        if (!assignmentId) {
          // Bridge: bind a null-section assignment to this student's section
        ...
        if (!assignmentId) {
          summary.errores++;
          failed.push(
            `${label}: no hay asignación del área en la sección del estudiante (créala en "Asignación de Áreas")`,
          );
          continue;
        }

        // 5) Upsert Definitiva Final (momento=0) → final_grades
        ...
        created.push(`${label}: ${item.grade_value}`);
      } catch (e) {
        summary.errores++;
    ...
    console.error("[import-bachillerato-grades] unexpected error:", error);
