...
function PlansTab({ schoolId }: { schoolId: string }) {
  ...
  const { data: plans = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("payment_plans").select("*, payment_plan_concepts(id, amount, display_order, is_mandatory, is_recurring, due_day, concept_id, payment_concepts(name, concept_type))").eq("school_id", schoolId).order("name");
      if (error) throw error;
      return data;
    },
  });
